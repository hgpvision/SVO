...
namespace svo {
	...
	public:
		...
		uint8_t patch_[patch_size_*patch_size_] __attribute__((aligned(16)));		//参考关键帧图像块，一维数组，8*8大小，按16字节对齐
		uint8_t patch_with_border_[(patch_size_ + 2)*(patch_size_ + 2)] __attribute__((aligned(16)));	//参考关键帧图像块（含边界），一维数组，10*10大小，按16字节对齐
		Matrix2d A_cur_ref_;          //!< affine warp matrix

		//在归一化平面（当前帧）上，搜索极线起点终点坐标之差，即搜索极线向量（注意是归一化图像坐标之差），不以像素为坐标
		//epi_dir_需乘以当前帧的内参矩阵获得像素坐标，方可得到epi_length_的值（极线长度，像素单位）
		Vector2d epi_dir_;
