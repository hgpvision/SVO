...
#define SUBPIX_VERBOSE 0
		...
		//
		bool align2D(
			const cv::Mat& cur_img,			//当前帧的指定层图像
			uint8_t* ref_patch_with_border,	//参考帧图像块，带边界
			uint8_t* ref_patch,				//参考帧图像块，不带边界
			const int n_iter,				//最大迭代次数
			Vector2d& cur_px_estimate,		//当前帧中特征点坐标（在cur_img中的坐标，并已经从底层变换到特征点在当前帧所在层中的坐标）
			bool no_simd)					//
		{
#ifdef __ARM_NEON__
			if (!no_simd)
